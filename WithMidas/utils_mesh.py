    ...
    while end_idx < len(end_pts):
        ...
        if len(end_pt) == 2:
            ...
            re_npath_map1, re_fpath_map1 = (
                (npath_map1 != -1).astype(np.uint8),
                (fpath_map1 != -1).astype(np.uint8),
            )
            re_npath_map2, re_fpath_map2 = (
                (npath_map2 != -1).astype(np.uint8),
...
def get_MiDaS_samples(
    image_folder, depth_folder, config, specific=None, aft_certain=None,inputImgNames=['input_Cam080', 'input_Cam000']
):
    # # lines = [
    ...
    for seq_dir in lines:
        ...
        # H, W = cv2.imread(sdict['ref_img_fi']).shape[:2]
